...
class TimedTest(unittest.TestCase):
    ...
    def test_a_run_that_took_no_time_is_not_read_as_infinitely_fast(self) -> None:
        ...
        found = speed.Timed("identify", 100, [0.0])
...
class ReportTest(unittest.TestCase):
    def test_the_report_names_the_rate_the_floor_and_the_runtime(self) -> None:
        lines = speed.lines_for(speed.Timed("identify", 1000, [0.001]), floor=1000)
        held = "\n".join(lines)

        self.assertIn("identify", held)
    ...
    def test_a_run_under_the_floor_says_so(self) -> None:
        lines = speed.lines_for(speed.Timed("identify", 1, [1.0]), floor=1_000_000)
...
class MainTest(unittest.TestCase):
    ...
    def test_a_run_that_beats_the_floor_reports_success(self) -> None:
        code, output = self.run_main(repeats=1, calls=20, floor=1)

        self.assertEqual(code, 0)
        self.assertIn("identify", output)

    def test_a_floor_nothing_could_beat_fails_the_run(self) -> None:
        code, output = self.run_main(repeats=1, calls=20, floor=10**12)
    ...
    def test_the_floor_shipped_here_is_beaten_on_this_machine(self) -> None:
        ...
        code, _ = self.run_main(repeats=1, calls=200)
