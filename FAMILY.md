...
This member consumes another. Where a header sits is `snes-mapper`, which is a
git submodule on the import path rather than a published package, because finding
a header and rewriting one must never disagree about where to look.
...
which the readme says at the install step. This package carries no `[project]`
block and publishes nothing, because it is not installable from an index while it
consumes another member this way.
